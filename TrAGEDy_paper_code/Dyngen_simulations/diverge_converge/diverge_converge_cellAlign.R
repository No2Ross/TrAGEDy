...
c_obj <- readRDS("TrAGEDy_results/simulated_data/diverge_converge/diverge_converge_c.rds")
d_obj <- readRDS("TrAGEDy_results/simulated_data/diverge_converge/diverge_converge_d.rds")
...
dist_mtx <- as.matrix(dist(rbind(t(interScaledGlobalc$scaledData), t(interScaledGlobald$scaledData)), diag = T, upper = T))
dist_mtx <- dist_mtx[ -which( row.names(dist_mtx) %in% colnames() ) , ]
dist_mtx <- dist_mtx[ , -which( colnames(dist_mtx) %in% colnames(d_pseudobulk) ) ]
dist_mtx <- as.data.frame(t(dist_mtx))
heatmap(as.matrix(dist_mtx), Rowv = NA, Colv = NA, scale="none")
